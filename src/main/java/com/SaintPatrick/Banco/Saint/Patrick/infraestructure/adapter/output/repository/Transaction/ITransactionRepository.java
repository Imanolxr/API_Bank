...
public interface ITransactionRepository extends JpaRepository<TransactionEntity, Long> {
    @Query("SELECT t FROM TransactionEntity t WHERE MONTH(t.dateTime) = :month AND (t.originCardEntity.cardNumber = :cardNumber OR t.destinyCardEntity.cardNumber = :cardNumber)")
    List<Transaction> findByMonthAndCardNumber(@Param("month") int month, @Param("cardNumber") String cardNumber);
}
